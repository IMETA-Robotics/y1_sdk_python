...
def generate_launch_description():
    robot_name_in_model = 'y1'
    package_name = 'y1_description'
    urdf_name = "y1_with_gripper.xacro"
    ...
    load_joint8_trajectory_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 
             'joint8_controller'],
        output='screen'
        )
    ...
    close_evt2 = RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[load_joint_trajectory_controller, 
                         ...
                         load_joint8_trajectory_controller],
            )
    )

    node_gripper_mirror_controller = Node(
        package='y1_gazebo',
        executable='y1_joint8_ctrl.py',
        output='screen'
    )
